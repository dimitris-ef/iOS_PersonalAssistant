...
grep -rn "TODO-DEVICE" --include=*.swift .                   # 7  — compiles, never executed
```
...
| --- | --- | --- |
| Swift Tests | macos-26 | The full suite compiles and runs, including the routing, identity, escalation, recurrence and permission mappings, and the EventKit and UserNotifications code (which compiles on macOS) |
| Apple SDK Check | macos-26 | EventKit, UserNotifications, AlarmKit and FoundationModels are present in the SDK, are really referenced by the built binary, and the two iOS 26 frameworks are **weakly** linked — the check that stops a launch crash on every iPhone below iOS 26 |
