...
public struct NativeFileStore: Sendable {
    ...
    /// A directory under `Application Support`.
    ///
    /// The subdirectory is a parameter because two unrelated kinds of model now
    /// live on the phone. `Models/` holds llama.cpp weights and
    /// `Models/Speech/` holds Whisper weights, and keeping them apart is what
    /// makes "delete the speech model" a operation that cannot touch the
    /// assistant's model — the two never share a directory listing.
    public static func applicationSupport(
        subdirectory: String,
        fileManager: FileManager = .default
    ) throws -> NativeFileStore {
        ...
        for component in subdirectory.split(separator: "/") {
            directory = directory.appendingPathComponent(String(component), isDirectory: true)
        }
        return NativeFileStore(directory: directory)
    }
    ...
    public static func temporary(
        ...
        NativeFileStore(
            directory: FileManager.default.temporaryDirectory
                .appendingPathComponent("\(prefix)-\(name)", isDirectory: true)
        )
    }
    ...
    public func prepareDirectory() throws {
        ...
        if fileManager.fileExists(atPath: directory.path, isDirectory: &isDirectory) {
            ...
            throw NativeDownloadError.diskWrite(
        ...
        do {
            ...
            throw NativeDownloadError.diskWrite(reason: error.localizedDescription)
        }
