...
public struct ReminderStage: Identifiable, Hashable, Codable, Sendable {
    ...
    /// Whether iOS currently holds a request for this stage.
    ///
    /// Orthogonal to `state`, and section 12 is why: `state` is what became of
    /// the reminder for the user, this is whether the OS was ever successfully
    /// asked to deliver it. A stage can be `pending` and undelivered — because
    /// notification permission is off — and saying so is more useful than
    /// either lying about it or destroying its lifecycle.
    public var delivery: StageDelivery
    ...
    public init(
        ...
        delivery: StageDelivery = .planned
    ) {
        self.id = id
        self.kind = kind
        self.offset = offset
    ...
    ///
    /// A stage that stops being pending stops needing an OS request, so its
    /// delivery state is withdrawn here rather than at each of the half-dozen
    /// call sites. What that does *not* do is cancel the request — that is
    /// `PlatformScheduleReconciler`'s job, and it happens after the plan has
    /// been persisted (section 59).
    public mutating func transition(to state: ReminderStageState, at date: Date) {
        self.state = state
        self.stateChangedAt = date
        if !state.isPending { delivery.withdrawn() }
    }

    /// True when the app should be asking iOS to deliver this.
    ///
    /// Pending, dated, and not already refused permanently.
    public var wantsDelivery: Bool {
        state.isPending && scheduledFor != nil && delivery.state != .unavailable
    }
}
...
public struct ReminderPlan: Identifiable, Hashable, Codable, Sendable {
    ...
    /// How many times this plan has been changed.
    ///
    /// ## What it is for
    ///
    /// Section 17 and section 18. A reminder scheduled with iOS is a promise
    /// made at a moment in time, and the plan can move underneath it: the event
    /// shifts, the user snoozes, a preparation timeline is recompressed. When
    /// that happens the old OS request describes a time that is no longer true.
    ///
    /// The revision is what makes an obsolete callback recognisable. A
    /// notification scheduled at revision 2 carries "2" in its payload; if the
    /// plan is at 4 when the user finally taps it, the app knows the button
    /// they pressed belonged to a plan that no longer exists and can decline it
    /// safely instead of resurrecting a stage that was replaced.
    ///
    /// Monotonic, never reused, and bumped by ``touch(at:)`` rather than by
    /// hand at each mutation site.
    ...
    /// Records that the plan has changed.
    ///
    /// Called once per decision rather than once per mutated stage: a
    /// recalculation that moves three stages is one change to the plan, and
    /// bumping three times would make every in-flight notification look two
    /// revisions staler than it is.
    public mutating func touch() {
        revision += 1
    }

    /// Stages the app should currently be asking iOS to deliver.
    public var deliverableStages: [ReminderStage] {
        stages.filter(\.wantsDelivery)
    }
