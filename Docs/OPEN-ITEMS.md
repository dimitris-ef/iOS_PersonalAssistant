...
| 35a | **The Xcode project, scheme and `.app` are still named `PersonalAssistant`.** | The app is *MetisAI* everywhere a person can see it — display name, bundle identifiers, App Group, URL scheme, App Store Connect record. The target names are build-time strings that appear in no shipped artifact and in nothing App Store Connect reads. Renaming them would touch every one of the four existing workflows for no user-visible gain, which is the unrelated refactoring Part 14 was told not to do. | `project.yml` |
| 35b | **`altool` is used for upload, and Apple has deprecated it in favour of Transporter.** | It is still shipped with Xcode 26 and still the only supported command-line uploader that takes an App Store Connect API key without a GUI. When it is removed, the two `xcrun altool` steps are the only thing that changes — everything around them talks to the REST API directly. | `testflight-deploy.yml` |
| 35c | **The processing poll gives up after 45 minutes and fails the job.** | Apple offers no upper bound on processing. A run that times out has still uploaded successfully, which is why the failure message says so and why `build_number_override` exists — it lets a re-run confirm the build already in App Store Connect rather than consuming a new build number. | `BuildProcessingPoller.wait` |
| 35d | **`MARKETING_VERSION` is still `0.1`.** | Preserved deliberately: Part 14 was told not to change it, and App Store Connect accepts it. It has to become something meaningful before a public release, but nothing about TestFlight requires that today. | `project.yml` |
| 36a | **The 3B crash was diagnosed from timing, not from a device crash log.** The fix models the micro-batch compute buffer the preflight was missing, and caps the batch numbers llama.cpp defaults to. | The crash happened when a conversation *started*, not when the model loaded, and those are two different allocations: `llama_init_from_model` takes weights and KV, the compute buffers come lazily on the first `llama_decode` and are sized from `n_ubatch`. Nothing in the Local AI path force-unwraps, traps or calls `fatalError`, so there was no Swift-level crash to find — consistent with a jetsam kill, which is uncatchable and leaves nothing in-process to observe. Confirming jetsam over a `GGML_ASSERT` needs a device crash log, which this environment cannot obtain. The fix is right either way: the preflight was genuinely missing a term, and a 512-token micro batch on a phone GPU that is also drawing the interface is genuinely wrong. | `LocalInferenceConfiguration`, `LocalModelResourceEstimator` |
| 36b | **The prompt-token estimate is `characters / 4`, not a tokenizer count.** | The real tokenizer lives behind the runtime protocol and needs the model loaded. Reaching for it would couple the provider to llama.cpp for a number it needs only approximately, and add a round trip per turn. Four characters per token is the standard rough figure; the estimate rounds up everywhere and charges per-turn template overhead, so it errs toward trimming one turn too many. That direction costs a line of old conversation; the other direction costs the app. | `LocalPromptBudget.estimatedTokens` |
| 36c | **Nothing measures whether the new batch numbers are fast enough.** They are chosen to be safe, and safety was the failing property. | Tokens per second is a real-device question and this codebase claims no figure for any of it (§121). A benchmarking UI is explicitly out of scope (§106). If generation turns out to be too slow on a given device class, the numbers are six fields in one type with a device tier each. | `LocalInferenceConfiguration.forDevice` |
| 37a | **The diagnostic trail says where execution stopped, never why.** An unclean session with an open `context_create` is not evidence of an out-of-memory kill. | iOS does not hand an app the reason its predecessor died. A jetsam kill, a native `GGML_ASSERT`, a watchdog termination and the user swiping the app away are indistinguishable from inside the next process, and `MetricKit` — which sometimes carries more — is delivered on Apple's schedule, not after each crash, so it cannot be the primary mechanism. The wording, the UI and a test all hold the line at "where". | `LocalInferenceRecoverySummary` |
| 37b | **`runtimeGPUOffloadActive` means "compiled with Metal and asked for", not "measured".** | This build cannot interrogate ggml for which backend a given tensor landed on. The three facts that *can* be established honestly — compiled, requested, and the resulting `n_gpu_layers` — are logged separately, so a reader can see the chain rather than a single claim that would be doing more work than the evidence supports. | `LlamaCppRuntime.loadModel` |
| 37c | **`retrievedMemoryCount` is allowlisted but never set.** Section 89 asks for it; the provider cannot see it. | `ContextAssembler` folds the selected memories into the system prompt before an `AIRequest` exists, so from the provider they are indistinguishable from the rest of the instructions. The key is kept for whoever wires it from the engine. Filling the field with a number derived from a guess would be worse than leaving it empty. | `LocalModelProvider.promptMetadata` |
| 37d | **MetricKit is not integrated.** Section 52 allows it as optional; this pass did not add it. | The breadcrumb log is synchronous, local and available on the very next launch. MetricKit payloads arrive up to a day later, are not guaranteed for every termination, and would have added a second delivery path to reason about for information the trail already localises. Worth revisiting once the crash is understood, as a cross-check rather than a primary. | — |
| 37e | **`fsync` is used, not `F_FULLFSYNC`.** | `write(2)` is what makes a breadcrumb survive *process death* — the bytes are in the kernel page cache and no longer belong to the process. `fsync` covers power loss; `F_FULLFSYNC` covers power loss during the device's own cache flush, at a cost of tens of milliseconds on the line immediately before every native call. Paying that would change the timing of the thing being measured. | `LocalInferenceFileWriter.append` |
...
| Distribution | That the signed build **launches** on a real iPhone. A VALID build in App Store Connect is a build Apple could process, not one that runs | the whole app |
| Distribution | That the App Group container resolves on device. The profiles grant it and the signed binaries carry it, but nothing has called `containerURL(forSecurityApplicationGroupIdentifier:)` on this bundle | `SharedStoreLocation` |
| Distribution | That the keyboard appears in Settings › General › Keyboards and types with Full Access **off** | `KeyboardViewController` |
| Distribution | That Live Activities start now `NSSupportsLiveActivities` is declared | `AppleLiveActivityCoordinator` |
| Distribution | That `BGTaskScheduler.register` matches the Info.plist identifier at launch. A mismatch traps rather than warns; both now derive from one constant, but the registration has never run | `AppLifecycleCoordinator` |
| Distribution | That `metisai://` deep links open the app from a widget tap | `SystemSurfaceDestination` |
...
| TestFlight Deploy | macos-26 | The Release configuration signs against three real distribution profiles; the archive and the exported IPA carry the right bundle identifiers, build number and **signed** App Group entitlement; the IPA contains no build credentials; and App Store Connect finishes processing that exact build as `VALID` |
